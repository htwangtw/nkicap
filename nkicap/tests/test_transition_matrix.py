...
def test_restore_transition_mat():
    ...
    drop_col = [f"from_cap{i+1:02d}_to_cap{j+1:02d}"
            ...
            for j in range(n_cap)
            if i == j]
    cur_flatten_data = pd.DataFrame(tm.flatten(), columns=["test"], index=labels_flatten).T
    cur_flatten_data = cur_flatten_data.drop(columns=drop_col)

    rtm = restore_transition_mat(cur_flatten_data, n_cap=n_cap)
